...
async function getFiltersForUserId(userId) {
...
async function getGrants({
    ...
    const { data, pagination } = await knex(TABLES.grants)
        ...
        .modify((queryBuilder) => {
            ...
            if (filters) {
                helpers.whereFiltersMatch(queryBuilder, filters);
            }
...
async function getTotalGrants({ filters } = {}) {
    const rows = await knex(TABLES.grants).modify(helpers.whereFiltersMatch, filters).count();
    return rows[0].count;
}
